...
def delete_repositories(api_token, org_name, prefix):
    ...
    response = requests.get(
        f"https://quay.io/api/v1/repository?public=false&namespace={org_name}",
        headers=headers,
    )
        ...
        for repo in repositories:
            ...
            if repo_name.startswith(prefix):
                delete_response = requests.delete(
                    f"https://quay.io/api/v1/repository/{org_name}/{repo_name}",
                    headers=headers,
                )
                if delete_response.status_code == 204:
                    ...
                    print(
                        f"Failed to delete repository: {full_repo_name}. Error message: {error_message}"
                    )
    else:

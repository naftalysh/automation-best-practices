...
def delete_robots(api_token, org_name, prefix):
    ...
    response = requests.get(
        f"https://quay.io/api/v1/organization/{org_name}/robots", headers=headers
    )
        ...
        for robot in robots:
            ...
            if robot_name.startswith(prefix):
                delete_response = requests.delete(
                    f"https://quay.io/api/v1/organization/{org_name}/robots/{robot_name}",
                    headers=headers,
                )
                if delete_response.status_code == 204:
                    print(f"Successfully deleted robot: {full_robot_name}")
                else:
                    error_message = json.loads(delete_response.text).get(
                        "message", "An unknown error occurred."
                    )
                    print(
                        f"Failed to delete robot: {full_robot_name}. Error message: {error_message}"
                    )
    else:
...
def main():
    ...
    parser.add_argument(
        "prefix", help="Prefix of the robot accounts you want to delete"
    )
